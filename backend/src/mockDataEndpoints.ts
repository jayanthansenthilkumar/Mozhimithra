...
const lessonsContent: Record<string, any> = {
    'l1': {
        id: 'l1',
        title: 'Saying Hello',
...
mockRouter.get('/user', (c) => c.json(userData));
mockRouter.get('/courses', (c) => c.json(courses));
mockRouter.get('/courses/:id', (c) => {
    const course = courses.find(course => course.id === c.req.param('id'));
    return course ? c.json(course) : c.json({ message: 'Course not found' }, 404);
});
mockRouter.get('/leaderboard', (c) => c.json(leaderboard));
mockRouter.get('/vocabulary', (c) => c.json(vocabulary));
mockRouter.get('/lessons/:id', (c) => {
    const lesson = lessonsContent[c.req.param('id')];
    return lesson ? c.json(lesson) : c.json({ message: 'Lesson not found' }, 404);
});
